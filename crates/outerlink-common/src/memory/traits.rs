...
/// Errors that can occur when pinning or unpinning pages.
...
pub enum PinError {
    ...
    /// Page is already pinned (for pin) or not pinned (for unpin).
    #[error("page vpn={0:#x} invalid pin state")]
    InvalidState(u64),

    /// Too many pages are pinned, cannot pin more.
    #[error("pin limit reached")]
    PinLimitReached,
}

/// Type of memory access, used for notify_access in TierManager.
...
pub enum AccessType {
    ...
    /// A write access.
    Write,
    /// An atomic (read-modify-write) access.
    Atomic,
}
...
pub trait PageTable: Send + Sync {
    ...
    /// Returns `Ok(())` on success, or a `MigrationError` if the page
    /// was not found or in an invalid state.
    fn commit_migration(
        &self,
        vpn: u64,
        new_tier: TierId,
    ...
    /// Set the parity group ID on an existing entry (R15 fault tolerance).
...
pub trait TierManager: Send + Sync {
    /// Allocate pages in a tier, returning the allocation result.
    fn allocate(
        &self,
        alloc_id: u32,
        size_bytes: usize,
    ...
    /// Deallocate a previous allocation by its ID.
    fn deallocate(&self, alloc_id: u32) -> Result<(), AllocationError>;

    /// Request promotion of a page to a higher (faster) tier.
    /// Returns the target tier if promotion is possible.
    fn request_promotion(&self, vpn: u64) -> Option<TierId>;

    /// Request demotion of a page from the given tier.
    /// Returns the VPN and target tier if demotion is needed.
    ...
    /// Get the remaining migration budget (bytes) between two tiers.
...
pub trait AccessMonitor: Send + Sync {
    /// Record a memcpy operation with VPN ranges and direction.
    fn record_memcpy(
        &self,
        dst_vpn_start: u64,
        src_vpn_start: u64,
...
pub trait CompressionHook: Send + Sync {
    /// Try to compress page data. Returns compressed bytes and compression
    /// ratio (compressed/original), or None if compression would not save space.
...
pub trait DedupHook: Send + Sync {
    /// Check if a page with the given data already exists.
    /// Returns the VPN of the canonical copy if a duplicate is found.
    fn check_dedup(&self, vpn: u64, data: &[u8]) -> Option<u64>;

    /// Notify that a deduplicated page reference has been freed.
