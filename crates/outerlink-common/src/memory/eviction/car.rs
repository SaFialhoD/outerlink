...
impl EvictionPolicy for CarPolicy {
    ...
    fn select_victim(&self, count: usize) -> Vec<EvictionCandidate> {
        ...
        let t1_len = self.t1.len();
        let prefer_t1 = t1_len > 0 && (t1_len as f64 >= self.p || self.t2.is_empty());

        // For each clock buffer, simulate a scan from the hand position
        // without mutating state. We look for entries with ref_bit=false
        // that are not pinned.
        let scan_clock = |clock: &[ClockEntry],
                          hand: usize,
            ...
            let len = clock.len();
            let mut h = hand % len;
            // Track entries that get a "virtual second chance" (ref_bit=true).
            ...
            for _ in 0..2 * len {
                ...
                let entry = &clock[h];
                if !pinned.contains(&entry.vpn) && !seen.contains(&entry.vpn) {
                    let effective_ref = if cleared.contains(&h) {
                        false
                    } else {
                        entry.ref_bit
                    };
                    ...
                    if effective_ref {
                        ...
                        seen.insert(entry.vpn);
                        candidates.push(EvictionCandidate {
                            vpn: entry.vpn,
                            tier_id: tier,
                            score: candidates.len() as f64,
                            dirty: false,
                        });
                    }
                }
