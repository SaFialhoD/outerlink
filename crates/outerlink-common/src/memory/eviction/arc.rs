...
impl ArcPolicy {
    ...
    /// Returns the evicted VPN, or None if T1 is empty or all pinned.
...
impl EvictionPolicy for ArcPolicy {
    ...
    fn select_victim(&self, count: usize) -> Vec<EvictionCandidate> {
        if (self.t1.is_empty() && self.t2.is_empty()) || count == 0 {
            return Vec::new();
        }
        ...
        // Determine which list to prefer based on ARC policy.
        let t1_len = self.t1.len();
        let prefer_t1 =
            t1_len > 0 && (t1_len as f64 > self.p || (self.t2.is_empty() && t1_len > 0));

        let (primary, secondary) = if prefer_t1 {
            (&self.t1, &self.t2)
        } else {
            (&self.t2, &self.t1)
        };

        // Scan primary list (LRU from front).
        for &vpn in primary.iter() {
            if candidates.len() >= count {
                break;
            }
            if !self.pinned.contains(&vpn) && !seen.contains(&vpn) {
                seen.insert(vpn);
                // Score: position in LRU order (lower = older = better candidate).
                let score = candidates.len() as f64;
                candidates.push(EvictionCandidate {
                    vpn,
                    tier_id: self.source_tier,
                    score,
                    dirty: false, // ARC does not track dirty state internally
                });
            }
        }

        // If more needed, scan secondary list.
        for &vpn in secondary.iter() {
            if candidates.len() >= count {
                break;
            }
            if !self.pinned.contains(&vpn) && !seen.contains(&vpn) {
