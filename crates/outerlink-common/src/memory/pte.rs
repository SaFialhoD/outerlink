...
/// in OuterLink's memory tiering system. Fields are ordered by decreasing
/// alignment to eliminate all padding under `repr(C)` layout rules:
///
/// | Offset | Size | Field                    |
/// |--------|------|--------------------------|
/// | 0      | 8    | vpn: u64                 |
/// | 8      | 4    | flags: PteFlags (u32)    |
/// | 12     | 4    | access_count: u32        |
/// | 16     | 4    | last_access_ts: u32      |
/// | 20     | 4    | alloc_id: u32            |
/// | 24     | 4    | parity_group_id: u32     |
/// | 28     | 2    | coherency: u16           |
/// | 30     | 2    | migration_count: u16     |
/// | 32     | 2    | last_migration_ts_delta  |
/// | 34     | 2    | prefetch_next_vpn_delta  |
/// | 36     | 2    | ref_count: u16           |
/// | 38     | 1    | tier_id: u8              |
/// | 39     | 1    | node_id: u8              |
/// | 40     | 1    | preferred_tier: u8       |
/// | 41     | 1    | access_pattern_type: u8  |
/// | 42     | 6    | phys_pfn: [u8; 6]        |
/// | 48     | 16   | dedup_hash: [u8; 16]     |
/// | Total  | 64   |                          |
#[repr(C, align(64))]
#[derive(Clone, Copy)]
pub struct PageTableEntry {
    ...
    pub alloc_id: u32,
    /// Parity/erasure coding group ID for fault tolerance (R15).
    pub parity_group_id: u32,

    // 2-byte aligned
    /// I/S/E coherency state + sharer bitmap.
    pub coherency: CoherencyField,
    /// How many times this page has been migrated between tiers.
    pub migration_count: u16,
    /// Delta (seconds) since the last migration timestamp, for recency tracking.
    pub last_migration_ts_delta: u16,
    /// Signed delta to the next predicted VPN for prefetching (R11).
    /// Negative values indicate backward prefetch direction.
